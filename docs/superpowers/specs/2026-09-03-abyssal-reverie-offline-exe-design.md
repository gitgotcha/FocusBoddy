...
第一版不包含：登录、注册、云同步、远程后端、自动更新、多窗口、代码签名。

> **范围变更（2026-09-04）**：系统托盘与后台运行已从"非目标"提升为已实现能力，因其是桌面版的核心体验（关闭窗口后计时必须继续可靠）。
> - 关闭主窗口 → 隐藏到托盘（不退出进程），运行中专注会话继续走时。
> - 托盘图标菜单：状态行（禁用）、暂停/继续、结束本次（不计入，沿用 2026-09-04 的停止不计入语义）、显示窗口、退出。
> - 托盘左键/双击 → 显示并聚焦窗口；右键 → 菜单。
> - 单实例：二次启动聚焦已有窗口而非新进程（tauri-plugin-single-instance）。
